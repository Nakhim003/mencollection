                                    ...
                                    @foreach (json_decode($product->images, true) as $index => $image)
                                        <div class="swiper-slide product-single__image-item">
                                            <img loading="lazy" class="h-auto"
                                                ...
                                                height="674" alt="Product Image {{ $index }}" />
                            ...
                            <div class="swiper-container thumbnail-swiper">
                                <div class="swiper-wrapper">
                                    ...
                                    @foreach (json_decode($product->images, true) as $index => $image)
                                        <div class="swiper-slide product-single__image-item">
                                            <img loading="lazy" class="h-auto"
                                                src="{{ asset('uploads/products/' . $image) }}" width="104"
                                                height="104" alt="Thumbnail Image {{ $index }}" />
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        @if (Cart::instance('wishlist')->content()->where('id', $product->id)->count() > 0)
                            <form method="POST"
                                ...
                                <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist filled-heart"
                                    onclick="document.getElementById('frm_remove_item').submit();">
                            ...
                            <div class="swiper-slide product-card">
                                <div class="pc__img-wrapper">
                                    <a href="{{ route('shop.products.details', ['product_slug' => $rproduct->slug]) }}">
                                        ...
                                        @if ($rproduct->images)
                                            @foreach (array_filter(array_map('trim', explode(',', $rproduct->images))) as $gimg)
                                                @php
                                                    $cleanImg = trim($gimg, '[]"');
                                                    $imageUrl = asset('uploads/products/' . $cleanImg);
                                                @endphp
                                    ...
                                    @else
                                        <form name="addtocart-form" method="post" action="{{ route('cart.add') }}">
                                            ...
                                            <input type="hidden" name="price"
                                                value="{{ $rproduct->sale_price == '' ? $rproduct->sale_price : $rproduct->regular_price }}" />
                                            <button type="submit"
                                ...
                                <div class="pc__info position-relative">
                                    ...
                                    <h6 class="pc__title"><a
                                            href="{{ route('shop.products.details', ['product_slug' => $rproduct->slug]) }}">{{ $rproduct->name }}</a>
                                    </h6>
                                    ...
                                    @if ($rproduct->reviews_count > 0)
                                        <div class="product-card__review d-flex align-items-center">
                                            ...
                                            <span
                                                class="reviews-note text-lowercase text-secondary ms-1">{{ $rproduct->reviews_count }}
                                                reviews</span>
                                        </div>
                                    @endif
                                    <button
